...
class GetPendingActivitiesUseCase @Inject constructor(
    private val repository: ActivitiesRepository
) {
    suspend operator fun invoke(): Flow<DataState<List<ActivityEntity>>> = flow {
        emit(DataState.Loading)
        val pendingActivities = repository.getPendingActivities()
        emit(DataState.Success(data = pendingActivities))
    }.catch { e ->
        e.printStackTrace()
